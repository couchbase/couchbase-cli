...
Server. This command used only be used when the cipher have been configured
in a way that breaks remote access. The cipher suit can be managed using the
man:couchbase-cli-settinig-security[1] command.
...
required, as the cluster and ciphers suits could be configured in away
