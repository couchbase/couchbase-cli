...
_couchbase-cli bucket-edit_ [--cluster <url>] [--username <user>]
    ...
    [--throttle-reserved <number>] [--throttle-hard-limit <number>]
    [--continuous-backup-enabled <0|1>] [--continuous-backup-location <location>]
    [--continuous-backup-interval <minutes>] [--continuous-backup-retention-period <hours>]
    [--continuous-backup-km-key-url <url>] [--continuous-backup-km-cred-id <cred-id>]
    [--continuous-backup-cloud-storage-cred-id <cred-id>]
...
--throttle-reserved <number>::
Measured in units per second. The guaranteed resource usage that is reserved for this
bucket only.

--throttle-hard-limit <number>::
Measured in units per second. If set, the bucket will be throttled at this limit
regardless of any capacity left over.

--continuous-backup-enabled <0|1>::
Enables or disables continuous backup for this bucket. Set to "1" to enable
or "0" to disable.

--continuous-backup-location <location>::
The location where continuous backup data will be stored.

--continuous-backup-interval <minutes>::
This interval determines the most recent restore time - e.g. an interval of two
minutes means you can restore to up to two minutes ago or any point in time
before (up to the retention period). The interval must be two minutes or above.
Short intervals will use more disk space.

--continuous-backup-retention-period <hours>::
The retention period for continuous backup data. Setting this to 0 disables
continuous backup retention. The maximum value is 3600 hours (60 days).

--continuous-backup-km-key-url <url>::
The KMS key URL used for continuous backup encryption. Supported schemes
include "awskms://", "gcpkms://", and "azurekeyvault://".

--continuous-backup-km-cred-id <cred-id>::
The credential ID for the key management service used with continuous backup
encryption.

--continuous-backup-cloud-storage-cred-id <cred-id>::
The credential ID for cloud object storage used with continuous backup.
