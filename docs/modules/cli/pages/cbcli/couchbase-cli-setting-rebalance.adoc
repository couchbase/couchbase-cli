...
_couchbase-cli setting-rebalance [--cluster <url>] [--username <user>] [--password <password>]
    ...
    [--rebalance-id <id>] [--file-based-throttle-rate <rate>] [--file-based-moves-per-node <num>]
...
--file-based-throttle-rate <rate>::
  The rate to throttle the file based rebalances to in bytes/s. Default is 0
  (no throttling).

--file-based-moves-per-node <num>::
  Specify the number of concurrent vBuckets to move per node during a file
  based rebalance. A higher setting may improve rebalance performance, at the
  cost of higher resource consumption.
