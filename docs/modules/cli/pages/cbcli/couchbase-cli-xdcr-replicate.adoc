...
_couchbase-cli xdcr-replicate_ [--cluster <url>] [--username <user>] [--password <password>]
    ...
    [--filter-deletions-with-expression <1|0>]
    [--filter-expirations-with-expression <1|0>] [--forward-local-only <1|0>]
    ...
    [--conflict-logging-pause-repl-threshold <num>]
    [--conflict-logging-monitor-duration <seconds>]
...
--filter-deletion <1|0>::
  ...
  operations on the source cluster. However, if
  `--filter-deletions-with-expression` is also set then deletions matching
  the expression _will_ be replicated.
...
--filter-expiration <1|0>::
  ...
  expirations on the source cluster. However, if
  `--filter-expirations-with-expression` is also set then expirations
  matching the expression _will_ be replicated.
...
--filter-deletions-with-expression <1|0>::
  When set to true, the filter expression will also be applied to delete
  mutations. This means deletions will only be sent to the target cluster if
  they match the filter expression. This option must be specified together
  with `--filter-deletion`.

--filter-expirations-with-expression <1|0>::
  When set to true, the filter expression will also be applied to expiry
  mutations. This means expirations will only be sent to the target cluster if
  they match the filter expression. This option must be specified together
  with `--filter-expiration`.

--forward-local-only <1|0>::
  When set to true, only mutations made locally on this cluster will be sent to
  the target cluster. Mutations this cluster received from another cluster via
  XDCR will not be forwarded on.
...
--conflict-logging-pause-repl-threshold <num>::
  The number of conflicts, per source cluster node, that may be logged within
  `--conflict-logging-monitor-duration` seconds before the replication is
  paused. By default XDCR stops logging conflicts for a period of time when
  too many are logged, so that its ability to replicate documents is
  protected. Setting a threshold instead pauses the replication once it is
  exceeded.

--conflict-logging-monitor-duration <seconds>::
  The period, in seconds, over which the logged conflicts are counted when
  deciding whether the replication should be paused. Used together with
  `--conflict-logging-pause-repl-threshold`.
