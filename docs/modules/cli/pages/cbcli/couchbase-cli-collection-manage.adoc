...
_couchbase-cli collection-manage_ [--cluster <url>] [--username <user>]
    ...
    [--max-ttl <seconds>] [--use-bucket-ttl] [--enable-history-retention <0|1>]
...
--max-ttl <seconds>::
  ...
  Cannot be used in conjunction with --use-bucket-ttl.

--use-bucket-ttl::
  Sets the maximum TTL (time-to-live) for all documents in the collection to
  the bucket's max TTL value, if it exists. Cannot be used in conjunction with
  --max-ttl.
