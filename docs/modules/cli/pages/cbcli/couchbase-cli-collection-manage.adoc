...
_couchbase-cli collection-manage_ [--cluster <url>] [--username <user>]
    ...
    [--delete-scope <scope>] [--list-scopes] [--create-collection <collection>]
    [--delete-collection <collection>] [--list-collections <scope>]
    [--max-ttl <seconds>]
...
To delete the couches collection from the chairs scope in the
furniture bucket.

  $ couchbase-cli collection-manage -c 192.168.1.5 -u Administrator \
   -p password --bucket furniture --delete-collection chairs.couches


To delete the chair scope from the furniture bucket

  $ couchbase-cli collection-manage -c 192.168.1.5 -u Administrator \
   -p password --bucket furniture --delete-scope chairs
