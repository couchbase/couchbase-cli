...
ifndef::doctype-manpage[:doctitle: backup-serivce-repository]
...
--bucket-name <name>::
    When adding an repository a bucket name can be supplied so that the repository only
    backups that bucket.
...
the underlaying data. This argument can only be used with archived repositories as imported
