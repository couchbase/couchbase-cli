...
--type <type>::
  The type of the link, either 'couchbase', 's3', 'azureblob'.
...
--shared-access-signature <token>::
  The shared access signature of the link. Only a single authentication method can be
used at a time. Authentication method can be connection string, account name &
account key, or account name & shared access signature.
...
----
To create an S3 link named `myAwsLink` in the "test" scope, with
...
----
To create an AzureBlob link named `myAzureBlobLink` in the "test" scope, using
*"connection string"* for authentication, run the following command:
----
$ couchbase-cli analytics-link-setup -c 192.168.1.5 -u Administrator \
 -p password --create --scope test --name myAzureBlobLink --type azureblob \
 --connection-string myConnectionString
----
To create an AzureBlob link named `myAzureBlobLink` in the "test" scope, using
*"account name and account key"* for authentication, run the following command:
----
$ couchbase-cli analytics-link-setup -c 192.168.1.5 -u Administrator \
 -p password --create --scope test --name myAzureBlobLink --type azureblob \
 --account-name myAccountName --account-key myAccountKey
----
To create an AzureBlob link named `myAzureBlobLink` in the "test" scope, using
*"account name and shared access signature"* for authentication, run the following command:
----
$ couchbase-cli analytics-link-setup -c 192.168.1.5 -u Administrator \
 -p password --create --scope test --name myAzureBlobLink --type azureblob \
 --account-name myAccountName --shared-access-signature mySharedAccessSignature
----
To create an AzureBlob link named `myAzureBlobLink` in the "test" scope, using
*"account name and shared access signature"* for authentication, and using a *custom
endpoint*, run the following command:
----
$ couchbase-cli analytics-link-setup -c 192.168.1.5 -u Administrator \
 -p password --create --scope test --name myAzureBlobLink --type azureblob \
 --account-name myAccountName --shared-access-signature mySharedAccessSignature \
 --blob-endpoint my-custom-endpoint.com
----
