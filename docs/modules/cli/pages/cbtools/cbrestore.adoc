...
resolution behavior is the same as cross datacenter replication (XDCR), which
...
Then we can restore only the first two by using the following command:
    $ cbrestore ~/backups http://10.112.193.101:8091 -u Administrator \
      -p passsword -b beer-sample --from-date 2019-06-25 --to-date 2019-06-27

The tool also gices the capability of filtering documents based on keys, giving
