...
class DataSetItem:
    def __init__(self, key, val=b'rawbytes', cas=0, expiry=0, data_type=0x00):
        self.key = key
        self.val = val
        self.cas = cas
        self.expiry = expiry
        ...
        self.exp_val = val
        self.exp_cas = cas
        self.exp_expiry = data_type
        self.exp_skip = False
        self.exp_data_type = data_type

    def set_expected(self, val=None, cas=None, expiry=None, data_type=None):
        if val:
            self.exp_val = val
        if cas:
            self.exp_cas = cas
...
def get_XATTR_pair(key, i):
    ...
    if key == b'txn':
        xattr_body = json.dumps({'cas': i, 'exp': i, 'id': str(i)}).encode()
...
class TestFilterOutTXN(unittest.TestCase):
    ...
    def generate_test_set(data_set_type, size):
        ...
        for i in range(size):
            ...
            if data_set_type == DataSetType.NO_XATTRS:
                ...
                item.data_type = cbcs.DATATYPE_HAS_XATTR
                item.set_expected(cas=i, expiry=i, data_type=0x00)
                ...
                item.set_expected(cas=i, expiry=i, val=get_value_with_XATTR(xattrs, b'rawbytes'),
                                  data_type=cbcs.DATATYPE_HAS_XATTR)
                item.data_type = cbcs.DATATYPE_HAS_XATTR
    ...
    def run_data_set(self, data_set):
        for el in data_set:
            skip, val, cas, exp, data_type = MCSink.filter_out_txn(el.key, el.val, el.cas, el.expiry, el.data_type)
            match, err = el.match(skip, val, cas, exp, data_type)
