...
class SettingOnDemand(Subcommand):
    """"Command to enable/configure on demand pricing as well as to generate usage tracking reports"""

    def __init__(self):
        super(SettingOnDemand, self).__init__()
        self.parser.prog = "couchbase-cli setting-on-demand"
        group = self.parser.add_argument_group("On demand pricing settings")
        group.add_argument('--set', dest='set', required=False, action="store_true",
                           help='Set licensing settings')
        group.add_argument('--get', dest='get', required=False, action="store_true",
                           help='Retrieve current licensing related settings')
        group.add_argument('--generate-report', dest='report', required=False, action="store_true",
                           help='Return RAM/core usage report')
        group.add_argument('--reporting-enabled', dest='reporting_enabled', metavar="<1|0>", choices=["1", "0"],
                           help="Enable on demand pricing")
        group.add_argument('--contract-id', dest='contract_id', metavar="<id>", help="Contract id")
        group.add_argument('--customer-token', dest="customer_token", metavar="<token>", help="Customer token")
    ...
    def execute(self, opts):
        ...
        _exitIfErrors(errors)
        if not enterprise:
            _exitIfErrors(['This command is ony available in enterprise edition'])

        flags_used = sum([opts.set, opts.get, opts.report])
        if flags_used == 0:
            _exitIfErrors(['Please provide one of --set, --get or --generate-report'])
        elif flags_used > 1:
            _exitIfErrors(['Please provide only one of --set, --get or --generate-report'])

        if opts.reporting_enabled == '1':
            opts.reporting_enabled = 'true'
        elif opts.reporting_enabled == '0':
            opts.reporting_enabled = 'false'

        if opts.get:
            self._list(rest, opts.output == 'json')
        elif opts.set:
            self._configure(rest, opts.reporting_enabled, opts.contract_id, opts.customer_token)
        elif opts.report:
            self._report(rest)

    @staticmethod
    def _list(rest, json_out):
        licensing_info, errors = rest.get_licensing_settings()
        _exitIfErrors(errors)
        if json_out:
    ...
    def _configure(rest, reporting_enabled, contract_id, customer_token):
        if reporting_enabled is None and contract_id is None and customer_token is None:
            _exitIfErrors(['At least one of --reporting-enabled, --contract-id or --customer-token is required'])

        # validate options and user credentials
        _, errors = rest.generate_licensing_report('false', reporting_enabled, contract_id, customer_token)
        _exitIfErrors(errors)
        # set options
        _, errors = rest.set_licensing_settings(reporting_enabled, contract_id, customer_token)
        _exitIfErrors(errors)
        _success('Licensing settings updated')

    @staticmethod
    def _report(rest):
        report, errors = rest.generate_licensing_report('true')
        _exitIfErrors(errors)
